export interface FrontendEventFrame {
  type: string;
  payload: Record<string, unknown>;
}

export interface FrontendEventsClient<E> {
  events: E[];
  comments: string[];
  close: () => void;
}
...
export async function connectFrontendEvents<
  E extends { type: string } = FrontendEventFrame,
>(port: number, session: string): Promise<FrontendEventsClient<E>> {
  ...
  if (res.status !== 200) {
    controller.abort();
    throw new Error(`frontend events connect failed with status ${res.status}`);
  }
  ...
  const client: FrontendEventsClient<E> = {
    events: [],
    comments: [],
...
// A tool call whose card reached a phase, as the frontend would see it. The
